...
exports.justify = (text) => {
  ...
  while (i < wordsLength) {
    ...
    if (words[j] == "\n") {
      ...
      if (numberOfWords == 1 || j >= wordsLength)
    ...
    while (
      j < wordsLength &&
      lineLength + words[j].length + (j - i - 1) < MAX_LENGTH
    ) {
...
/**
 * It adds the spaces to the middle of the text.
 * @param {string[]} words
 * @param {int} diff
 * @param {int} i
 * @param {int} j
 * @returns an array contains lines with spaces in the middle
 */
...
/**
 * It added the spaces to the right of the text.
 * @param {string[]} words
 * @param {int} diff
 * @param {int} i
 * @param {int} j
